...
public static class AobPatterns
{
    ...
    ///   <c>48 39 2D ^ ?? ?? ?? ?? 0F 85 ?? ?? ?? ?? B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8B F8 48 89 44 24 50</c>
    ...
    /// A patch changed the jnz displacement (was a hard-coded <c>16 01 00 00</c>) and the
    /// allocator immediate; both are wild-carded. Uniqueness comes from the post-call
    /// <c>mov rdi,rax / mov [rsp+50],rax</c> tail.
    /// </summary>
