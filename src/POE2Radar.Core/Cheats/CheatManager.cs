...
public sealed class CheatManager : ICheatPatchTarget
{
    private readonly ProcessHandle _process;
    private readonly MemoryReader _reader;
    private readonly Dictionary<string, CheatState> _cheats = new(StringComparer.Ordinal);
    ...
    public void ScanAndResolve()
    {
        ...
        foreach (var def in CheatDefinition.All())
        {
            ...
            {
                var pattern = def.Pattern.AsSpan();
                var matches = AobScanner.FindPattern(bytes, pattern);
    ...
    private void ResolveConstantAddress(CheatState state, nint sectionBase, byte[] sectionBytes, int matchOffset)
    {
        ...
        if (probe == null)
        {
            Console.WriteLine($"  patch [{def.ShortName}] constant at 0x{candidate:X} unreadable");
            return;
        }
        ...
        {
            Console.WriteLine($"  patch [{def.ShortName}] constant at 0x{candidate:X} invalid ({origFloat})");
            return;
        }

        state.ConstantAddress = candidate;
        Console.WriteLine($"  patch [{def.ShortName}] constant at 0x{candidate:X} = {origFloat}");
    }

    public void SyncFrom(GamePatchSettings settings) => CheatPatchSync.Apply(settings, this);

    public bool SetEnabled(string name, bool enabled)
    {
        if (!_cheats.TryGetValue(name, out var state) || !state.Found) return false;
        if (enabled)
            return state.Definition.Type == CheatType.PatchConstant
                ? SetConstantValue(name, state.CurrentValue)
                : Enable(state);
        return Disable(state);
    }

    public bool IsActive(string name) =>
        _cheats.TryGetValue(name, out var state) && state.Active;

    public bool TryGetCurrentValue(string name, out float value)
    {
        if (_cheats.TryGetValue(name, out var state))
        {
    ...
    {
        var result = new Dictionary<string, CheatInfo>(StringComparer.Ordinal);
    ...
    private bool WriteBytes(nint address, byte[] bytes)
    {
        ...
        {
            Console.WriteLine($"  patch: VirtualProtectEx failed at 0x{address:X}");
            return false;
        }
        ...
        if (!ok) Console.WriteLine($"  patch: WriteProcessMemory failed at 0x{address:X}");
        return ok;
    }
}
